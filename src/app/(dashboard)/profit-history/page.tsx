...
export default function ProfitHistoryPage() {
  ...
  const CustomTooltip = ({ active, payload, label }: any) => {
    ...
    return (
      <div style={{ background: '#161B26', border: '1px solid #1F2937', borderRadius: '0.75rem', padding: '0.75rem 1rem', fontSize: '0.8rem' }}>
        ...
        {payload[1] && <p style={{ color: '#A78BFA', fontWeight: 600 }}>Vendas: {formatCurrency(payload[1].value)}</p>}
      </div>
    )
  }
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
        <div style={{ display: 'flex', background: '#161B26', borderRadius: '0.875rem', padding: '0.3rem', gap: '0.25rem' }}>
          {(['dia', 'semana', 'mes'] as const).map(p => (
            <button key={p} onClick={() => setPeriod(p)} style={{ padding: '0.4rem 1rem', borderRadius: '0.625rem', background: period === p ? '#1F2937' : 'transparent', border: 'none', color: period === p ? '#F1F5F9' : '#94A3B8', cursor: 'pointer', fontSize: '0.8rem', fontWeight: period === p ? 600 : 400, transition: 'all 0.15s', textTransform: 'capitalize' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: '1rem' }}>
        {[
          ...
          { label: `Lucro médio por ${period}`, value: formatCurrency(avg), icon: Calendar, color: '#A78BFA', bg: 'rgba(99,102,241,0.12)' },
      ...
      <div className="glass" style={{ borderRadius: '1rem', padding: '1.5rem' }}>
        ...
        {loading ? (
          <div style={{ display: 'flex', justifyContent: 'center', padding: '3rem' }}><div style={{ width: '2rem', height: '2rem', border: '3px solid #161B26', borderTopColor: '#10B981', borderRadius: '50%', animation: 'spin 0.8s linear infinite' }} /><style>{`@keyframes spin{to{transform:rotate(360deg)}}`}</style></div>
        ) : (
